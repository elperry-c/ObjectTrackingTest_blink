...
void setup(){
  ...
  for (int i=0; i<4; i++){
    turnTiming[i] = 150 + i * 30;
  }
  turnTiming[4] = 150; // turnTiming = [150, 180, 210, 240, 270, 150]
  // デバッグ用 //////////////
  for (int i=0; i<5; i++){
  ...
  openEyes = loadImage("_jpg_files/open-eye.jpg");
  closedEyes = loadImage("_jpg_files/closed-eye.jpg");
}
...
void common(){
  ...
  fill(255);
  textSize(24);
  image(openEyes, 0, 0, height/6, height/6);
  /* 
...
void play_scene(){
  ...
  // closed eye
  if (fcnt%120 < 5 || fcnt%120 > 116){
    image(closedEyes, 0, 0, height/6, height/6);
  }
